...
public class Jazz extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        jazz1 = getResources().getStringArray(R.array.available_jazz1);
        jazz2 = getResources().getStringArray(R.array.available_jazz2);
        jazz3 = getResources().getStringArray(R.array.available_jazz3);
        TextView textView1=(TextView) findViewById(R.id.textView1);
        textView1.setText(Arrays.toString(jazz1).replace("["," ").replace("]"," ").trim());
        TextView textView2=(TextView) findViewById(R.id.textView2);
        textView2.setText(Arrays.toString(jazz2).replace("["," ").replace("]"," ").trim());
        TextView textView3=(TextView) findViewById(R.id.textView3);
        textView3.setText(Arrays.toString(jazz3).replace("["," ").replace("]"," ").trim());
        ...
        switch(view.getId()) {
            case R.id.checkBoxSong1:
                if (checked) { //add it to the set
                    setValues.add(Arrays.toString(jazz1));

                }
                else //remove it from the set
                {
                    setValues.remove(Arrays.toString(jazz1));
                }
                break;

            case R.id.checkBoxSong2:
                if (checked) {
                    setValues.add(Arrays.toString(jazz2));
                }
                else
                {
                    setValues.remove(Arrays.toString(jazz2));
                }
                break;

            case R.id.checkBoxSong3:
                if (checked){
                    setValues.add(Arrays.toString(jazz3));
                }

                else{
                    setValues.remove(Arrays.toString(jazz3));
                }
