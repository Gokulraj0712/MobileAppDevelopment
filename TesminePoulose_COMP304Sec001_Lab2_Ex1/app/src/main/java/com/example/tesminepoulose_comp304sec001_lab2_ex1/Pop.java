...
public class Pop extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        pop1 = getResources().getStringArray(R.array.available_pop1);
        pop2 = getResources().getStringArray(R.array.available_pop2);
        pop3 = getResources().getStringArray(R.array.available_pop3);
        TextView textView1=(TextView) findViewById(R.id.textView1);
        textView1.setText(Arrays.toString(pop1).replace("["," ").replace("]"," ").trim());
        TextView textView2=(TextView) findViewById(R.id.textView2);
        textView2.setText(Arrays.toString(pop2).replace("["," ").replace("]"," ").trim());
        TextView textView3=(TextView) findViewById(R.id.textView3);
        textView3.setText(Arrays.toString(pop3).replace("["," ").replace("]"," ").trim());
        ...
        songType = myPref.getString("songType","Pop");
        ...
        switch(view.getId()) {
            case R.id.checkBoxSong1:
                if (checked) { //add it to the set
                    setValues.add(Arrays.toString(pop1));

                }
                else //remove it from the set
                {
                    setValues.remove(Arrays.toString(pop1));
                }
                break;

            case R.id.checkBoxSong2:
                if (checked) {
                    setValues.add(Arrays.toString(pop2));
                }
                else
                {
                    setValues.remove(Arrays.toString(pop2));
                }
                break;

            case R.id.checkBoxSong3:
                if (checked){
                    setValues.add(Arrays.toString(pop3));
                }

                else{
                    setValues.remove(Arrays.toString(pop3));
                }
