...
public class Classical extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        class1 = getResources().getStringArray(R.array.available_classical1);
        class2 = getResources().getStringArray(R.array.available_classical2);
        class3 = getResources().getStringArray(R.array.available_classical3);
        TextView textView1=(TextView) findViewById(R.id.textView1);
        textView1.setText(Arrays.toString(class1).replace("["," ").replace("]"," ").trim());
        TextView textView2=(TextView) findViewById(R.id.textView2);
        textView2.setText(Arrays.toString(class2).replace("["," ").replace("]"," ").trim());
        TextView textView3=(TextView) findViewById(R.id.textView3);
        textView3.setText(Arrays.toString(class3).replace("["," ").replace("]"," ").trim());
        ...
        switch(view.getId()) {
            case R.id.checkBoxSong1:
                if (checked) { //add it to the set
                    setValues.add(Arrays.toString(class1));

                }
                else //remove it from the set
                {
                    setValues.remove(Arrays.toString(class1));
                }
                break;

            case R.id.checkBoxSong2:
                if (checked) {
                    setValues.add(Arrays.toString(class2));
                }
                else
                {
                    setValues.remove(Arrays.toString(class2));
                }
                break;

            case R.id.checkBoxSong3:
                if (checked){
                    setValues.add(Arrays.toString(class3));
                }

                else{
                    setValues.remove(Arrays.toString(class3));
                }
